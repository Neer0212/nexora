...
export default async function ProductsPage() { const supabase=await createClient(); const {data:{user}}=await supabase.auth.getUser(); if(!user) redirect("/login"); const data=await getEntityInsights(); return <AppShell><ProductOverview currencyCode={data.currencyCode} products={data.products}/></AppShell> }
