...
export default async function Page(){let d;try{d=await getInvestigateData()}catch(e){if(e instanceof Error&&e.message==="AUTH_REQUIRED")redirect("/login");if(e instanceof Error&&e.message==="BUSINESS_REQUIRED")redirect("/onboarding");throw e}return <AppShell><InvestigateOverview kind="projects" rows={d.rows} columns={d.columns} currencyCode={d.currencyCode}/></AppShell>}
